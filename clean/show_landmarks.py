...
def extract_landmarks(image, offset_history):
    wrist_coords = None
    fingertip_coords = []
    nose_coords = None
    offsets = {}
    ...
    # Apply smoothing to offsets
    smoothed_offsets = {}
    for key, value in offsets.items():
        offset_history[key].append(value)
        if len(offset_history[key]) >= 5:  # Ensure we have enough data points to smooth
            smoothed_x = smooth_data([p[0] for p in offset_history[key]])
            smoothed_y = smooth_data([p[1] for p in offset_history[key]])
            smoothed_offsets[key] = [smoothed_x[-1], smoothed_y[-1]]
        else:
            smoothed_offsets[key] = value

    return image, smoothed_offsets

# Function to record gestures and save training data
def record_gesture_training_data(cap, num_entries=10):
    training_data = []
    entry_count = 0
    offset_history = {f'{finger}_offset': deque(maxlen=10) for finger in ["Thumb", "Index", "Middle", "Ring", "Pinky", "Nose"]}
    ...
    while entry_count < num_entries:
        ...
        frame_with_landmarks, offsets = extract_landmarks(frame_rgb, offset_history)
...
if __name__ == '__main__':
    ...
    mode = 'v'# input("Enter 'r' to record gestures or 'v' to view landmarks: ").strip().lower()
    offset_history = {f'{finger}_offset': deque(maxlen=10) for finger in ["Thumb", "Index", "Middle", "Ring", "Pinky", "Nose"]}
